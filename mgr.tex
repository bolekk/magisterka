...
% S??owa kluczowe:
\keywords{systemy P2P, systemy rozproszone, replikacja danych, DHT, backup}

\keywordsang{P2P storage systems, distributed systems, data replication, DHT, backup, resource management}
...
Distributed storage systems are designed to overcome limitations of traditional, organizationally centralized data stores.
...
Another interesting deployment is presented in \cite{hetero}. A backup system similar to Nebulostore was installed in a university computer lab and also in the PlanetLab environment. Authors claim that the main factor contributing to system's quality and efficiency is the availability of hosts. In the university lab deployment, the average availability of the machines was only 13\% and transient failures were very frequent. However, with the help of asynchronous messaging, auhors were able to achieve overall stability. Average times of creating the first, the second and the third replica of a file ware equal to, respectively, 1.1h, 2.7h and 5.5h.\\
