...
In this chapter we analyse various approaches to data replication in peer-to-peer systems, categorized with respect to the degree of centralization. We begin with a discussion regarding the model of peers' availability. Afterwards, we present some of the existing results and also propose a new approach.\\
...
We can observe that this model lacks the symmetry of the probabilistic model. Distribution of availability slots can lead to situations where peer A is worthless for peer B but very valuable for peer C.\\ (TODO: example, image)

MIARA - tu moze byc zlozona bo jednostronna

\section{General assumptions}

1. Kazdy replikuje wielokrotnosc jakiejs jednostki ilosci danych (w wiekszosci przypadkow wszyscy po tyle samo ale nie koniecznie). Jesli ktos chce wiecej to moze miec kilka "tozsamosci". Czy to jest stale dla wszystkich peeerow - TAK Kazdy tez serwuje wlasne dane czy nie?\\

2. Nikt nie zawyza bo weryfikacja. Jestesmy w stanie zweryfkowac zawyzanie. Czy zanizanie tez?\\

3. Wszyscy sie znaja dzieki centralnemu serwerowi,? .\\

Musimy miec miare dal calego systemu, ktora jest sprawiedliwa (paper).\\ Mamy tez miary dla peerow.\\
...
Results presented in the previous section suggest a conclusion that centralized optimization approaches to the replication problem can be computationally-heavy and also impractical for a few other reasons. We would like to investigate a new, decentralized approach where peers communicate with one another and agree or disagree to replicate one another's data.\\

Since the time-slot model is not symmetric with respect to relative peers' contributions, we will not be relying on the idea of clique-based replication. \\
...
Nie chcemy  pelnej decentralizacji bo faworyzuje lepszych.\\

W hybrydzie zakladamy, ze wszyscy wiedza o istniejacych peerach dzieki centralym metadanym. Nadal pozwalamy peerom na dowolne algorytmy dobierania sie.\\

Rezygnujemy z wzajemnosci.\\
