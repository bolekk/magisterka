...
4. {\bf Corporate($\alpha$)}. The corporate setting is taken from \cite{krz} and is supposed to better reflect the real time zone distribution, which is not uniform. All peers are available in a continuous interval of slots, of which 10\% is available in 8 slots, 25\%  in 7 slots, 30\% in 6 slots and the remaining 35\% in only 2 slots. Furthermore, the starting slot is chosen randomly from a Pareto distribution with the shape parameter equal to $\alpha$ for $\alpha \in \{0.1, 0.5, 1.0, 1.5, 2.0\}$. Additionally, the list of time-slots is randomly permuted before selecting slots.\\

\subsection{Results}

Results for the {\bf uniform} setting are TODO.\\

Results for the {\bf uniform continuous} setting are depicted in figures \ref{uni_cont_1}, \ref{uni_cont_3} and \ref{uni_cont_5}. \\
...
\caption{Results for the uniform continuous setting with $K=5$ replication slots. (TODO!)}
\label{uni_cont_5}
\end{figure}

Napster scenario is shown on figure \ref{napster}.

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{graphs/napster.pdf}
\caption{Results for Napster}
\label{napster}
\end{figure} 
 
In the last of our test scenarios, named {\it corporate}, we test five values for the distribution shape parameter $\alpha$. 
 
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{graphs/pareto_5_slots.pdf}
\caption{Results for Pareto with K = 5 replication slots.}
\label{pareto_5_slots}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{graphs/corp.pdf}
\caption{Results for Pareto (FIXME!)}
\label{pareto_5_slots}
\end{figure}
