...
public class CorpUniformPeerFactory implements PeerFactory {

  private final Random random;

  public CorpUniformPeerFactory(long randomSeed) {
  ...
  public List<Peer> generatePeers(int nPeers, int T, int slots,
    ...
    addRandomPeers(peers, (int) (nPeers * 0.1), T, 0.95, slots, privateMeasure,
        acceptanceMeasure);
    addRandomPeers(peers, (int) (nPeers * 0.25), T, 0.87, slots,
        privateMeasure, acceptanceMeasure);
    addRandomPeers(peers, (int) (nPeers * 0.3), T, 0.75, slots, privateMeasure,
        acceptanceMeasure);
    int rest = nPeers - (int) (nPeers * 0.1) - (int) (nPeers * 0.25)
        - (int) (nPeers * 0.3);
    addRandomPeers(peers, rest, T, 0.33, slots, privateMeasure,
        acceptanceMeasure);
  ...
  private void addRandomPeers(List<Peer> peers, int n, int T, double prob,
    ...
    int id = peers.size();
    for (int i = 0; i < n; ++i) {
      peers.add(new Peer(id++, T, getRandomAv(T, (int) (T * prob)), slots,
          privateMeasure, acceptanceMeasure));
    }
  }
